...
def _register_gateway_auto_approve(session_key: str) -> None:
    """Auto-answer any pending gateway approval for ``session_key`` with
    'once'. Mirrors the helper in
    ``tests/cron/test_scheduler_cron_session_isolation.py`` so we can prove the
    untrusted path actually goes through the human-decision engine."""
...
class TestTrustedExecuteCodeProfileLane:
    ...
    A gateway/ask profile whose name appears in
    ``approvals.trusted_execute_code_profiles`` skips the whole-script pending
    approval. Cron, unattended platforms, and yolo/off-mode precedence stay
    UNCHANGED — the lane is a narrow, config-visible opt-in, never a global
    bypass.
    """
    ...
    def _gateway_context(self, monkeypatch, session_key="test-trusted-profile-session"):
        monkeypatch.delenv("HERMES_CRON_SESSION", raising=False)
        monkeypatch.delenv("HERMES_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("HERMES_INTERACTIVE", raising=False)
        monkeypatch.delenv("HERMES_EXEC_ASK", raising=False)
    ...
    def test_trusted_profile_auto_approves_in_gateway_context(self, monkeypatch):
        """A gateway profile allowlisted in trusted_execute_code_profiles skips
        the pending prompt entirely — no gateway notify_cb registered, yet the
        gate returns approved without going through the human-decision engine."""
        from tools.approval import check_execute_code_guard

        self._isolate(monkeypatch)
        session_key = self._gateway_context(monkeypatch)
        # Active profile is "nagatha" and the allowlist contains it.
        monkeypatch.setattr(
        ...
        # Deliberately do NOT register _register_gateway_auto_approve: a real
        # pending approval would block for the full timeout. If the trusted
        # lane works, the gate never queues one.
        try:
            result = check_execute_code_guard("import os", "local")
            assert result["approved"] is True
            # Approved by the trust lane, not a user round-trip.
    ...
    def test_untrusted_profile_still_prompts(self, monkeypatch):
        """An active profile that is NOT in the allowlist still flows through
        the human approval path. We prove it via the registered gateway notify
        callback: the untrusted path must reach the gateway queue, and the
        callback's auto-approve must drive the answer (user_approved=True)."""
        from tools.approval import check_execute_code_guard

        self._isolate(monkeypatch)
        session_key = self._gateway_context(monkeypatch)
        # Active profile is "operator", allowlist only contains "nagatha".
        monkeypatch.setattr(
        ...
        try:
            result = check_execute_code_guard("import os", "local")
            assert result["approved"] is True
            # Went through the human approval path, not the trust lane.
    ...
    def test_cron_denies_even_for_trusted_profile(self, monkeypatch):
        """Cron sessions DENY regardless of the trust lane — the lane is
        gateway/ask only; cron deny is enforced by the earlier
        ``_unattended_contexts()`` gate, which the trusted lane sits behind."""
        from tools.approval import check_execute_code_guard

        self._isolate(monkeypatch)
        # Cron context, gateway env cleared.
        monkeypatch.delenv("HERMES_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("HERMES_INTERACTIVE", raising=False)
        monkeypatch.delenv("HERMES_EXEC_ASK", raising=False)
        monkeypatch.setenv("HERMES_CRON_SESSION", "1")
    ...
    def test_yolo_or_off_takes_precedence(self, monkeypatch):
        """YOLO / mode=off auto-approve is checked BEFORE the trust lane.
        An untrusted profile with yolo on must still be approved."""
        from tools.approval import check_execute_code_guard

        self._isolate(monkeypatch)
        session_key = self._gateway_context(monkeypatch)
        # Force yolo on; profile NOT trusted.
    ...
    def test_ask_mode_trusted_profile_auto_approves(self, monkeypatch):
        """Ask mode (HERMES_EXEC_ASK=1) is a whole-script approval context, so a
        trusted profile auto-approves there too — no human round-trip."""
        from tools.approval import check_execute_code_guard
    ...
    def test_trusted_profile_match_is_case_insensitive(self, monkeypatch):
        """Case-insensitive: allowlist ['NAGATHA'] trusts active profile 'nagatha';
        a non-matching profile is not trusted."""
        self._isolate(monkeypatch)
        monkeypatch.setattr(
            "hermes_cli.profiles.get_active_profile_name", lambda: "nagatha"
        )
    ...
    def test_malformed_allowlist_is_treated_empty(self, monkeypatch):
        """A non-list allowlist value must be treated as an empty allowlist (safe):
        it never grants trust to any profile."""
        monkeypatch.setattr(
            approval_context, "_get_approval_config",
    ...
    def test_profile_resolution_failure_is_safe_not_trusted(self, monkeypatch):
        """If get_active_profile_name() raises, the trust lane must NOT grant trust."""
